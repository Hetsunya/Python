...
def bubble(point, step, col):
    ...
    allcol = (sd.COLOR_RED, sd.COLOR_ORANGE, sd.COLOR_YELLOW, sd.COLOR_GREEN,
                      sd.COLOR_CYAN, sd.COLOR_BLUE, sd.COLOR_PURPLE)
    for _ in range(3):
        #radius += step
        sd.circle(center_position=point, radius=step, color=allcol[col], width=10)
...
for y in range(100, 301, 100):
    for x in range(100, 1001, 100):
        bubble(point=sd.get_point(x, y), step=5, col=4)
...
for _ in range(100):
    bubble(point=sd.random_point(), step=5, col=rnd.randint(0,6))
